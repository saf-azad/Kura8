...
export function installInteraction(config: Config): void {
  ...
  canvas.onpointerdown = e => {
    ...
    const node = handle ? config.selected() : [...doc.nodes].reverse().find(n => contains(n.rect, p));
  ...
  canvas.onpointermove = e => {
    const p = point(e);
    if (!drag) { const handle = handleAt(p); canvas.style.cursor = handle ? `${handle}-resize` : doc.nodes.some(n => contains(n.rect, p)) ? 'move' : 'default'; return; }
    const raw = resizeRect(drag.rect, drag.handle, p.x - drag.start.x, p.y - drag.start.y, drag.node.type === 'image');
    const result = config.snapping ? snap({ ...raw, handle: drag.handle, anchors: config.anchors, canvas: config.size, threshold: 8 / config.scale(), lockAspect: drag.node.type === 'image', dominantAxis: raw.dominantAxis }) : { rect: raw.rect, hits: [] };
  ...
  canvas.ondblclick = e => { const p = point(e), n = [...doc.nodes].reverse().find(n => contains(n.rect, p)); if (n) config.edit(n); };
}
