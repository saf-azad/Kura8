...
type Actions = { addText(): void; addRect(): void; addShape(shape: ShapeKind): void; toggleLock(): void; duplicate(): void; mac: boolean; addImage(): void; removeBackground(): void; delete(): void; export(): void; update(): void; selected(): DocNode | undefined; };
export function toolbar(host: HTMLElement, actions: Actions): { refresh(): void; exportButton: HTMLButtonElement; backgroundButton: HTMLButtonElement } {
  ...
  const background = button('Remove image background', icons.background, actions.removeBackground); background.className = 'background'; host.append(background);
  ...
  return { exportButton: exp, backgroundButton: background, refresh() {
    ...
    background.disabled = n?.type !== 'image' || !!n.locked || background.getAttribute('aria-busy') === 'true';
